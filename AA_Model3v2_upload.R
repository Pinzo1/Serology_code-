...
AB_range_bins_cs1 = matrix(NA, nrow=N_bins, ncol=3)				## generate an empty matrix to be filled with antibody levels for each age bin and the binomial CIs
...
model_M3 <- function(a, t_survey, par_M3)
{
	...
	if( a <= age_SD )
	{
		## time_0_prime  = time_0 - t_survey
		## alpha_0_prime = alpha_0
		## alpha_c_prime = alpha_0 + ((time_0-t_survey)/time_0)*(alpha_c-alpha_0)
		##
		## AB_titre = ((alpha_0_prime-alpha_c_prime)/(time_0_prime*rr*rr))*(1 - exp(-rr*a)) - (a*(alpha_0_prime-alpha_c_prime)/time_0_prime)*exp(-rr*a) + (alpha_c_prime/rr)*(1 - exp(-rr*a))
		##
		## The above expression can be analytically simplified to:

		AB_titre = ( (alpha_0-alpha_c)/(time_0*rr*rr) )*(1 - exp(-rr*a)) - 
                       ( a*(alpha_0-alpha_c)/time_0 )*exp(-rr*a) + 
                       ( alpha_c/rr )*(1 - exp(-rr*a)) + 
                       ( t_survey*(alpha_0-alpha_c)/(time_0*rr) )*(1 - exp(-rr*a))
	}

	if( a > age_SD )
	{
		#################################
		## Calculate antibody level at age of sexual debut

		AB_titre = ( (alpha_0-alpha_c)/(time_0*rr*rr) )*(1 - exp(-rr*age_SD)) - 
                       ( age_SD*(alpha_0-alpha_c)/time_0 )*exp(-rr*age_SD) + 
                       ( alpha_c/rr )*(1 - exp(-rr*age_SD)) + 
                       ( (t_survey+a-age_SD)*(alpha_0-alpha_c)/(time_0*rr) )*(1 - exp(-rr*age_SD))


		#################################
		## Now calculate how antibody level changes between 
		## age of sexual debut and time of survey

		time_0_prime  = a - age_SD  
		alpha_0_prime = alpha_STI + alpha_0 + ((time_0-t_survey-(a-age_SD))/time_0)*(alpha_c - alpha_0)
		alpha_c_prime = alpha_STI + alpha_0 + ((time_0-t_survey)/time_0)*(alpha_c - alpha_0)
		
		AB_titre = AB_titre*exp(-rr*(a-age_SD)) + 
                       ( (alpha_0_prime-alpha_c_prime)/(time_0_prime*rr*rr) )*(1 - exp(-rr*(a-age_SD))) - 
                       ( (a-age_SD)*(alpha_0_prime-alpha_c_prime)/(time_0_prime*rr) )*exp(-rr*(a-age_SD)) + 
	                 ( alpha_c_prime/rr )*(1 - exp(-rr*(a-age_SD)))
	}
...
plot(sapply(seq(from=0, to=60, length=1000), model_M3, t_survey=0, par_M3=c(0.1, 0.2, 1, 0.1)))
...
loglike_M3_cs1 <- function( par_M3 )
{
	...
	AB_model = sapply(AB_cs1[,1], model_M3, t_survey=0, par_M3=par_M3)
...
prior_M3 = function( par_M3 )
{
	...
	prior = prior_alpha_0 + prior_gamma +  prior_sigma + prior_alpha_STI + prior_rr
...
N_mcmc       <- 60000      ## Number of MCMC iterations
...
colnames(MCMC_par) = c("alpha_0", "gamma", "rr", "sigma", "alpha_STI", "loglike", "prior")
...
par_MC = c(10, 0.5, 0.2, 0.6, 10)       ## (alpha_0, gamma, rr, sigma, alpha_STI)
...
for(k in 1:N_sam)
{
	M3_sam_cs1[k,] = sapply(age_seq, model_M3, t_survey=15, par=MCMC_burn[sam_seq[k],1:5])
}

M3_quant_cs1 = matrix(NA, nrow=3, ncol=length(age_seq))
for(j in 1:length(age_seq))
{
	M3_quant_cs1[,j] = quantile( M3_sam_cs1[,j], prob=c(0.025, 0.5, 0.975), na.rm=TRUE )
}
...
plot(x=age_bins_mid, y=GMT_bins_cs1, 
pch=15, cex=2,
log="y", xlim=c(0,90), ylim=c(0.1,5000),
xlab="age (years)", ylab="Geometric mean antibody titre", 
main="Cross-sectional antibody data"  )
