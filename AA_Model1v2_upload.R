...
pch=15, cex=2,
log="y", xlim=c(0,20), ylim=c(0.1,100),
...
par_MC <- c(2, 0.55)  ## (alpha, rr, sigma)
...
colnames(MCMC_par) <- c("alpha", "sigma", "loglike", "prior")

 
#########################################################
## 2.3 Implement MCMC iterations

par_MC <- c(1, 0.5)                 ## Initial guess: (alpha, rr, sigma)
...
age_seq <- seq(from=0, to=10, by=0.5)
...
plot(x=age_bins_mid, y=GMT_bins, 
pch=15, cex=2,
log="y", xlim=c(0,20), ylim=c(0.1,500),
xlab="age (years)", ylab="Geometric mean antibody titre", 
