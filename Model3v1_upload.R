...
par_MC <- c(0.1, 0.5, 0.03)  ## (lambda_0, gamma, rho)
...
{ 
	...
	prior <- prior_lambda_0 + prior_gamma + prior_rho 
...
colnames(MCMC_par) <- c("lambda_0", "gamma", "rho", "loglike", "prior")
...
par_MC <- c(0.2, 0.1, 0.05)  ## (lambda_0, gamma, rho)
...
## 3.1 Plot data and model prediction
 pdf("Continued_decline_estSRR.pdf")
jpeg("Model3_RenBel.jpg")
par(mfrow=c(1,1))
...
xlim=c(0,90), ylim=c(0,1),
xlab="age (years)", ylab="Proportion seropositive", 
main="Sero-catalytic Model 3 fit - RenBel"  )
