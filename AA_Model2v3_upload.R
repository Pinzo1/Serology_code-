...
pch=15, cex=2,
log="y", xlim=c(0,70), ylim=c(0.01,200),
...
prior_M2 = function( par_M2 )
{
	...
	prior <- prior_alpha_0 + prior_gamma + prior_rr + prior_time_c + prior_sigma + prior_alpha_STI
...
MCMC_par           = matrix(NA, nrow=N_mcmc, ncol=8)
colnames(MCMC_par) = c("alpha_0", "gamma", "rr", "time_c", "sigma", "alpha_STI", "loglike", "prior")
...
par_MC = c(40, 0.5, 0.1, 10, 0.6, 20)       ## (alpha_0, gamma, rr, time_c, sigma, alpha_STI)
...
par_median = apply(X=MCMC_burn[,1:6], MARGIN=2, FUN=median)
...
for(k in 1:N_sam)
{
	M2_sam_cs[k,] = sapply(age_seq, model_M2, t_survey=0, par=MCMC_burn[sam_seq[k],1:6])
}
...
xlab="age (years)", ylab="Geometric mean antibody titre", 
main="Antibody acquisition Model 2 with STI - Rombo CT694"  )
...
theta_bar = apply( X=MCMC_burn[,1:6], FUN=mean, MARGIN=2)
...
D_bar = -2*mean( MCMC_burn[,7] - MCMC_burn[,8] )
...
theta_bar = apply( X=MCMC_burn[,1:6], FUN=median, MARGIN=2)
...
D_bar = -2*median( MCMC_burn[,7] - MCMC_burn[,8] )
