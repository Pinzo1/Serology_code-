...
par_MC <- c(5, 0.1,  10, 0.6)  ## (alpha_0, gamma, rr, time_c, sigma)

rr <- 0.091055			## define the fixed rate of antibody decay
...
prior_M2 <- function( par )
{
	...
	prior <- prior_alpha_0 + prior_gamma  + prior_time_c + prior_sigma
...
colnames(MCMC_par) <- c("alpha_0", "gamma", "time_c", "sigma", "loglike", "prior")
...
par_MC <- c(30, 0.5,  10, 0.6)       ## (alpha_0, gamma, rr, time_c, sigma)
...
age_seq <- seq(from=0, to=9, by=0.5)
...
log="y", xlim=c(0,90), ylim=c(0.1,200),
xlab="age (years)", ylab="Geometric mean antibody titre", 
main="Antibody acquisition Model 2 fix RR - Rombo CT694"  )
