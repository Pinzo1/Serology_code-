...
data <- data[,]										## you may need to subset the data according to region or ethnic group
...
pch=15, cex=2,
log="y", xlim=c(0,90), ylim=c(0.1,4000),
...
age_max = 80   ## maximum age in population
...
prior_M3 = function( par_M3 )
{
	...
	prior = prior_alpha_0 + prior_gamma  + prior_sigma
...
MCMC_par           = matrix(NA, nrow=N_mcmc, ncol=5)						## define empty matrix for MCMC output
colnames(MCMC_par) = c("alpha_0", "gamma",  "sigma", "loglike", "prior")
...
par_MC = c(40, 0.5,  0.6)       ## (alpha_0, gamma, sigma)
...
ylim=quantile( MCMC_par[,4], prob=c(0.01,1)),
main="likelihood" )
