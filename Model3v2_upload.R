...
par_MC <- c(0.1, 0.5, 0.03, 0.05)  ## (lambda_0, gamma, rho, lambda_STI)
...
{ 
	...
	prior <- prior_lambda_0 + prior_gamma + prior_rho + prior_lambda_STI
...
colnames(MCMC_par) <- c("lambda_0", "gamma", "rho", "lambda_STI", "loglike", "prior")
...
par_MC <- c(0.2, 0.1, 0.05, 0.03)  ## (lambda_0, gamma, rho, lambda_STI)
...
for(mc in 1:N_mcmc)
{
	...
	MCMC_par[mc,6]   <- prior_M3( par_MC )	## return the prior
}
...
pch=19, col="grey", cex=0.25,
xlab="MCMC iteration", ylab="lambda_STI", 
main="lambda_STI")
...
xlab="age (years)", ylab="Proportion seropositive", 
main="Sero-catalytic Model 3 fit with STI - RenBel"  )
