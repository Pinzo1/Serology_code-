...
setwd("") ## set the wd to where the data is
...
rho = 0.02611204			## rho is now a fixed parameter 
...
N_adapt      <- 5000	   ## End of adaptive scaling of proposal size with rm_scale in 2.1
step_scale  <- 0.001	   ## Scaler for step size
...
colnames(MCMC_par) <- c("lambda", "loglike", "loglike_data")

 
#########################################################
## 2.3 Implement MCMC iterations

par_MC <- c(0.05)  ## (lambda) ## initial paramater guess

Sigma_MC <- rbind( c(0.0000206, 0.00),		## Initial guess of covariance of MVN proposal dist
                   c(0.0, 0.0000036) ) 
...
for(mc in 1:N_mcmc)
{
	par_MCp1 <- mvrnorm(n=1, mu=par_MC, Sigma=step_scale)
	...
	if( par_MCp1[1] > 0  ){
		...
		#if( mc == N_tune )
		#{
			#Sigma_MC <- cov( MCMC_par[1:(N_tune-1),1:2] )
		#}
	}

	MCMC_par[mc,1]   <- par_MC				## store the output parameter values from each MCMC realisation 
	MCMC_par[mc,2]   <- loglike_MC			## return the associated loglikelihood
	MCMC_par[mc,3]   <- prior_M1(par_MC)	## return the prior
}
...
par_median <- median(MCMC_burn[,1])



age_seq <- seq(from=0, to=16, by=1)
...
D_bar = -2*mean( MCMC_burn[,2] - MCMC_burn[,3] )
...
D_bar = -2*median( MCMC_burn[,2] - MCMC_burn[,3] )
...
save.image("Model1_Christmas_fixSRR.RData")
