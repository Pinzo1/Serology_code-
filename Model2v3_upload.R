...
model_M2_STI = cmpfun(model_M2_STI, options=list(optimize=3)) 
...
prior_M2 = function( par_M2 )
{
	...
	prior = prior_lambda_0 + prior_gamma + prior_rho + prior_time_c + prior_lambda_STI
...
colnames(MCMC_par) = c("lambda_0", "gamma", "rho", "time_c", "lambda_STI", "loglike", "prior")
...
par_MC = c(0.1, 0.5, 0.1, 20, 0.2)       ## (lambda_0, gamma, rho, time_c, lambda_STI)
...
for(mc in 1:N_mcmc)
{
	...
	MCMC_par[mc,1:5] = par_MC			## store the output parameter values from each MCMC realisation 
	MCMC_par[mc,6]   = loglike_MC		## return the associated loglikelihood
	MCMC_par[mc,7]   = prior_MC			## return the prior
}
